...
def to_numeric(ser, dtype_backend='pyarrow'):
    ...
    if 'time' in dt or 'geometry' in dt:
        ...
        ser = pd.to_numeric(ser, errors='ignore', downcast='integer', dtype_backend=dtype_backend).convert_dtypes(dtype_backend=dtype_backend)  # cast to numeric nullable datatypes where possible
...
def prep(X, cap='casefold'):
    ...
    if X is None or X is np.nan:
        ...
        g = lambda df:df.apply(to_numeric).rename(columns={x:prep(x, cap).replace(' ','_').replace('-','_') if isinstance(x, str) else x for x in df.columns})
        idx = g(X[[]].reset_index())
